      ...
      <div className="page-header">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-3">
            {totals.expense > 0 && (
              <div className="card relative overflow-hidden flex flex-col gap-0.5 px-4 py-2"
                style={{ background: `color-mix(in srgb, #ef4444 12%, var(--bg-surface))`, borderColor: `color-mix(in srgb, #ef4444 28%, var(--border-default))` }}>
                <span className="absolute left-0 top-0 bottom-0 w-1 rounded-l-xl" style={{ background: '#ef4444' }} />
                <p className="kpi-label relative z-10">Expenses</p>
                <p className="kpi-value relative z-10 text-lg" style={{ color: '#ef4444' }}>{formatCurrency(totals.expense, sym)}</p>
              </div>
            )}
            {totals.saving > 0 && (
              <div className="card relative overflow-hidden flex flex-col gap-0.5 px-4 py-2"
                ...
                <p className="kpi-label relative z-10">Savings</p>
                <p className="kpi-value relative z-10 text-lg" style={{ color: '#3b82f6' }}>{formatCurrency(totals.saving, sym)}</p>
              </div>
            )}
            {totals.investment > 0 && (
              <div className="card relative overflow-hidden flex flex-col gap-0.5 px-4 py-2"
                style={{ background: `color-mix(in srgb, #8b5cf6 12%, var(--bg-surface))`, borderColor: `color-mix(in srgb, #8b5cf6 28%, var(--border-default))` }}>
                <span className="absolute left-0 top-0 bottom-0 w-1 rounded-l-xl" style={{ background: '#8b5cf6' }} />
                <p className="kpi-label relative z-10">Investments</p>
                <p className="kpi-value relative z-10 text-lg" style={{ color: '#8b5cf6' }}>{formatCurrency(totals.investment, sym)}</p>
              </div>
            )}
          </div>
